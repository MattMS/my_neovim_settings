...
syntax region v_quoted start='"' end='"' matchgroup=quote nextgroup=a_block,bad_indent,k_special,k_text oneline skipempty skipwhite
...
syntax match bad_indent '^\t* \+'

" syntax match v_text '[^ ]\+' nextgroup=a_block,k_special,k_text

" syntax match k_text '[^ ]\+' nextgroup=v_date,v_id,v_list,v_number,v_text

syntax match v_id '@[^ ]\+' nextgroup=a_block,bad_indent,k_special,k_text

syntax match v_number '\d\+' nextgroup=a_block,bad_indent,k_special,k_text

syntax match v_date '\d\{4\}-\d\{2\}-\d\{2\}' nextgroup=a_block,bad_indent,k_special,k_text

syntax match v_datetime '\d\{4\}-\d\{2\}-\d\{2\}T\d\{4\}' nextgroup=a_block,bad_indent,k_special,k_text

syntax match v_url 'http[^ ]\+' nextgroup=a_block,bad_indent,k_special,k_text

syntax keyword k_special act date note start stop

" highlight default link a_block Error

highlight default link bad_indent Error
